...
- `pi-agents/shared/` - Pi 共享 agents（已迁移），手动复制到 `~/.pi/agent/agents/` 使用
...
```bash
# 复制到 Pi 全局 agents 目录
cp ~/Code/opencode-skills/pi-agents/shared/*.md ~/.pi/agent/agents/
...
- Pi agent 新增/修改后，必须同步更新 `pi-agents/shared/` 和 `~/.pi/agent/agents/` 两处文件
