...
- **两级作用域** — 全局 Agent（`~/.pi/agent/agents/`）和项目 Agent（`.pi/agents/`）
...
- **全局 Agent**：`~/.pi/agent/agents/` — 所有项目可用
- **项目 Agent**：`.pi/agents/` — 仅当前项目可用
...
**代码审查员**（`~/.pi/agent/agents/code-reviewer.md`）：
