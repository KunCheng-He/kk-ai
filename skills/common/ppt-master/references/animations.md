...
uv run python skills/ppt-master/scripts/animation_config.py scaffold <project>

# Validate references before export
uv run python skills/ppt-master/scripts/animation_config.py validate <project>

# Export reads <project>/animations.json automatically when present
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project>
```
...
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> -t push --transition-duration 0.6

# Disable
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> -t none

# Auto-advance every 5 seconds (kiosk-style playback)
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> --auto-advance 5
```
...
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> -a auto

# Enable with a single effect (cascades via the after-previous trigger)
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> --animation fade

# Enable and switch to on-click for live presentations (presenter controls pacing)
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> -a auto --animation-trigger on-click

# Custom pacing
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> --animation mixed \
        --animation-stagger 0.7 --animation-duration 0.5

# All groups animate in unison on slide entry
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project> --animation-trigger with-previous
```
