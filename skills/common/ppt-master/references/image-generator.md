...
```bash
uv run python scripts/slice_images.py <project>/images/illus_sheet.png --grid 2x3 \
...
```bash
uv run python scripts/image_gen.py \
...
- Still run `uv run python scripts/image_gen.py --render-md project/images/image_prompts.json` so the human-readable sidecar exists without touching any backend.
