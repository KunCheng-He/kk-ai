...
```bash
uv run python scripts/image_search.py "<query>" \
  --filename <name>.jpg \
  --slide <slide_id> \
...
```bash
uv run python scripts/image_search.py --batch <project_path>/images/image_queries.json \
  -o <project_path>/images
```
...
2. **manual URL replace (universal, model-agnostic)** — the user finds a better image anywhere and gives its URL; download and swap it in:
   ```bash
   uv run python scripts/image_search.py --from-url <image-url> --filename <name>.jpg -o <project_path>/images
   ```
...
```bash
uv run python scripts/image_search.py "<query>" --filename <name>.jpg -o <project_path>/images \
...
```bash
uv run python scripts/image_search.py --promote candidate_03.jpg --filename <name>.jpg -o <project_path>/images
```
