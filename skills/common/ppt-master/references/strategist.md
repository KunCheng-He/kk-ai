...
> 6. **Copy each chosen icon into the project as you confirm it** — `uv run python skills/ppt-master/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
...
uv run python skills/ppt-master/scripts/latex_render.py <project_path>
```
...
**When selection includes B**, you must run `uv run python scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
```bash
uv run python scripts/project_manager.py init <project_name> --format <canvas_format>
```
