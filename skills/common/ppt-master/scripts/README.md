...
```bash
uv run python scripts/source_to_md.py <file-or-url-or-dir> [<file-or-url-or-dir> ...]
# or direct backend calls:
uv run python scripts/source_to_md/pdf_to_md.py <file.pdf>
# or
uv run python scripts/source_to_md/ppt_to_md.py <deck.pptx>
uv run python scripts/source_to_md/excel_to_md.py <workbook.xlsx>
uv run python scripts/project_manager.py init <project_name> --format ppt169
uv run python scripts/project_manager.py import-sources <project_path> <source_files_or_dirs...> --move
uv run python scripts/total_md_split.py <project_path>
uv run python scripts/finalize_svg.py <project_path>
uv run python scripts/animation_config.py scaffold <project_path>  # optional object-level animation overrides
uv run python scripts/svg_to_pptx.py <project_path>
```

Repository update:

```bash
uv run python scripts/update_repo.py
```
...
uv run python scripts/source_to_md/pdf_to_md.py <file.pdf>
uv run python scripts/source_to_md/ppt_to_md.py <deck.pptx>
uv run python scripts/source_to_md/doc_to_md.py <file.docx>
uv run python scripts/source_to_md/excel_to_md.py <workbook.xlsx>
uv run python scripts/source_to_md/web_to_md.py <url>
```
...
uv run python scripts/project_manager.py init <project_name> --format ppt169
uv run python scripts/project_manager.py import-sources <project_path> <source_files_or_dirs...> --move
uv run python scripts/project_manager.py validate <project_path>
```

Template source import:

```bash
uv run python scripts/pptx_template_import.py <template.pptx>
uv run python scripts/pptx_template_import.py <template.pptx> --manifest-only
uv run python scripts/pptx_template_import.py <template.pptx> --inheritance-mode both
```
...
```bash
uv run python scripts/project_manager.py init <project_name> --format ppt169
uv run python scripts/project_manager.py import-sources <project_path> <source.pptx> <material...>
# Manual fallback when import-sources did not produce analysis/<stem>.slide_library.json:
uv run python scripts/template_fill_pptx.py analyze <project_path>/sources/<source.pptx> -o <project_path>/analysis/<stem>.slide_library.json
uv run python scripts/template_fill_pptx.py scaffold <project_path>/analysis/<stem>.slide_library.json -o <project_path>/analysis/fill_plan.json --slides "1,3,4"
uv run python scripts/template_fill_pptx.py check-plan <project_path>/analysis/<stem>.slide_library.json <project_path>/analysis/fill_plan.json -o <project_path>/analysis/check_report.json
uv run python scripts/template_fill_pptx.py apply <project_path>/sources/<source.pptx> <project_path>/analysis/fill_plan.json -o <project_path>/exports/filled.pptx
uv run python scripts/template_fill_pptx.py validate <project_path>
```
...
```bash
uv run python scripts/native_enhance_pptx.py init <source.pptx> --name <project_slug>
uv run python scripts/native_enhance_pptx.py plan <project_path>
uv run python scripts/native_enhance_pptx.py validate <project_path>
uv run python scripts/native_enhance_pptx.py apply <project_path>
```

Post-processing and export:

```bash
uv run python scripts/extract_svg_assets.py <svg_dir> --icons-dir <icons_dir> --inplace --id-prefix <prefix>  # optional: shrink imported/reference SVGs before AI review
uv run python scripts/total_md_split.py <project_path>
uv run python scripts/finalize_svg.py <project_path>
uv run python scripts/svg_to_pptx.py <project_path>
```
...
```bash
uv run python scripts/latex_render.py <project_path>
uv run python scripts/latex_render.py <project_path> --providers codecogs,quicklatex,mathpad,wikimedia
uv run python scripts/image_gen.py "A modern futuristic workspace"
uv run python scripts/image_gen.py --list-backends
uv run python scripts/analyze_images.py <project_path>/images
```
...
```bash
uv run python scripts/update_repo.py
uv run python scripts/update_repo.py --skip-pip
```
