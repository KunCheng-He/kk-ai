...
```bash
uv run python scripts/confirm_ui/server.py <project_path> --daemon --wait   # launch + wait for Stage 1
uv run python scripts/confirm_ui/server.py <project_path> --wait-only --wait-stage stage2  # Stage 2: wait for the design-system handoff
uv run python scripts/confirm_ui/server.py <project_path> --wait-only       # Stage 3: wait for the final result
uv run python scripts/confirm_ui/server.py <project_path> --daemon
uv run python scripts/confirm_ui/server.py <project_path> --daemon --port 5051
uv run python scripts/confirm_ui/server.py <project_path> --no-browser
uv run python scripts/confirm_ui/server.py <project_path> --timeout 0   # disable idle auto-shutdown
uv run python scripts/confirm_ui/server.py <project_path> --shutdown    # Step 4 cleanup (idempotent)
```
