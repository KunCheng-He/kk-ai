...
```bash
uv run python scripts/source_to_md.py paper.pdf
uv run python scripts/source_to_md.py paper.pdf report.docx deck.pptx
uv run python scripts/source_to_md.py ./sources
uv run python scripts/source_to_md.py ./pdfs/*.pdf
uv run python scripts/source_to_md.py ./decks/*.pptx
uv run python scripts/source_to_md.py report.docx -o report.md
uv run python scripts/source_to_md.py ./sources -o ./markdown  # explicit separate output directory
uv run python scripts/source_to_md.py workbook.xlsx --json
uv run python scripts/source_to_md.py deck.pptx
uv run python scripts/source_to_md.py https://example.com/article -o article.md
```
...
```bash
uv run python scripts/source_to_md/pdf_to_md.py book.pdf
uv run python scripts/source_to_md/pdf_to_md.py book.pdf -o output.md
uv run python scripts/source_to_md/pdf_to_md.py book.pdf appendix.pdf
uv run python scripts/source_to_md/pdf_to_md.py ./pdfs
uv run python scripts/source_to_md/pdf_to_md.py ./pdfs -o ./markdown  # explicit separate output directory

# Image extraction control (default: filtered)
uv run python scripts/source_to_md/pdf_to_md.py book.pdf --images filtered  # size/quality filters applied
uv run python scripts/source_to_md/pdf_to_md.py book.pdf --images all       # extract all images, no filtering
uv run python scripts/source_to_md/pdf_to_md.py book.pdf --images none      # skip all images (text only)
```
...
```bash
# Install: cd scripts && uv sync
# Original: pip install PyMuPDF
```
...
```bash
uv run python scripts/source_to_md/doc_to_md.py lecture.docx
uv run python scripts/source_to_md/doc_to_md.py lecture.docx -o output.md
uv run python scripts/source_to_md/doc_to_md.py lecture.docx notes.html
uv run python scripts/source_to_md/doc_to_md.py ./docs
uv run python scripts/source_to_md/doc_to_md.py ./docs -o ./markdown  # explicit separate output directory
uv run python scripts/source_to_md/doc_to_md.py notes.epub
uv run python scripts/source_to_md/doc_to_md.py paper.tex -o paper.md  # uses pandoc
```
...
# Install: cd scripts && uv sync
# Original: pip install mammoth markdownify ebooklib nbconvert beautifulsoup4
...
```bash
uv run python scripts/source_to_md/excel_to_md.py report.xlsx
uv run python scripts/source_to_md/excel_to_md.py report.xlsx -o output.md
uv run python scripts/source_to_md/excel_to_md.py report.xlsx budget.xlsm
uv run python scripts/source_to_md/excel_to_md.py ./workbooks
uv run python scripts/source_to_md/excel_to_md.py ./workbooks -o ./markdown  # explicit separate output directory
uv run python scripts/source_to_md/excel_to_md.py report.xlsm --max-rows 200 --max-cols 40
```
...
```bash
uv run python scripts/source_to_md/ppt_to_md.py sales_deck.pptx
uv run python scripts/source_to_md/ppt_to_md.py sales_deck.pptx -o output.md
uv run python scripts/source_to_md/ppt_to_md.py sales_deck.pptx appendix.pptx
uv run python scripts/source_to_md/ppt_to_md.py ./decks
uv run python scripts/source_to_md/ppt_to_md.py ./decks -o ./markdown  # explicit separate output directory
uv run python scripts/source_to_md/ppt_to_md.py template.ppsx -o notes/template.md
```
...
```bash
# Install: cd scripts && uv sync
# Original: pip install python-pptx
```
...
```bash
uv run python scripts/pptx_intake.py deck.pptx -o projects/demo/analysis
```
...
```bash
uv run python scripts/source_to_md/web_to_md.py https://example.com/article
uv run python scripts/source_to_md/web_to_md.py https://url1.com https://url2.com
uv run python scripts/source_to_md/web_to_md.py -f urls.txt
uv run python scripts/source_to_md/web_to_md.py https://example.com -o output.md
uv run python scripts/source_to_md/web_to_md.py https://example.com --emit-result /tmp/result.json
```
...
```bash
uv run python scripts/rotate_images.py auto projects/xxx_files
uv run python scripts/rotate_images.py gen projects/xxx_files
uv run python scripts/rotate_images.py fix fixes.json
```
