...
uv run python scripts/latex_render.py <project_path> --dry-run
...
uv run python scripts/image_gen.py "Abstract tech background" --aspect_ratio 16:9 --image_size 4K
uv run python scripts/image_gen.py "Concept car" -o projects/demo/images
uv run python scripts/image_gen.py "Beautiful landscape" -n "low quality, blurry, watermark"
uv run python scripts/image_gen.py --list-backends
```

Backends are grouped into Core / Extended / Experimental tiers. Run `uv run python scripts/image_gen.py --list-backends` for the current list.

Backend selection:

```bash
uv run python scripts/image_gen.py "A cat" --backend openai
uv run python scripts/image_gen.py "A cinematic portrait" --backend minimax
uv run python scripts/image_gen.py "A product launch hero image" --backend qwen
uv run python scripts/image_gen.py "科技感背景图" --backend zhipu
uv run python scripts/image_gen.py "A product KV in cinematic style" --backend volcengine
```
...
```bash
uv run python scripts/image_search.py "offshore wind farm" \
...
```bash
# Pin Wikimedia
uv run python scripts/image_search.py "Olympics opening ceremony" \
  --filename event.jpg --provider wikimedia \
  --orientation landscape -o projects/demo/images

# Strict mode — refuse CC BY / CC BY-SA
uv run python scripts/image_search.py "abstract gradient" \
...
```bash
uv run python scripts/gemini_watermark_remover.py <image_path>
uv run python scripts/gemini_watermark_remover.py <image_path> -o output_path.png
uv run python scripts/gemini_watermark_remover.py <image_path> -q
```
...
# Original: pip install Pillow numpy
```
