...
uv run python scripts/project_manager.py import-sources <project_path> <source1_or_dir> [<source2_or_dir> ...]
uv run python scripts/project_manager.py validate <project_path>
uv run python scripts/project_manager.py info <project_path>
```
...
```bash
uv run python scripts/project_manager.py init my_presentation --format ppt169
uv run python scripts/project_manager.py validate projects/my_presentation_ppt169_20251116
uv run python scripts/project_manager.py info projects/my_presentation_ppt169_20251116
```
...
```bash
uv run python scripts/project_utils.py <project_path>
```
...
```bash
uv run python scripts/batch_validate.py examples
uv run python scripts/batch_validate.py examples projects
uv run python scripts/batch_validate.py --all
uv run python scripts/batch_validate.py examples --export
```
...
```bash
uv run python scripts/generate_examples_index.py
uv run python scripts/generate_examples_index.py examples
```
...
uv run python scripts/pptx_template_import.py <template.pptx> -o <output_dir>
uv run python scripts/pptx_template_import.py <template.pptx> --manifest-only
uv run python scripts/pptx_template_import.py <template.pptx> --skip-manifest
uv run python scripts/pptx_template_import.py <template.pptx> --embed-images
uv run python scripts/pptx_template_import.py <template.pptx> --inheritance-mode both
uv run python scripts/pptx_template_import.py <template.pptx> --inheritance-mode flat
uv run python scripts/pptx_template_import.py <template.pptx> --inheritance-mode layered
```
...
```bash
uv run python scripts/error_helper.py
uv run python scripts/error_helper.py missing_readme
uv run python scripts/error_helper.py missing_readme project_path=my_project
```
