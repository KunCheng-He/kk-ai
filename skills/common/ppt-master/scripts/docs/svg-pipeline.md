...
```bash
uv run python scripts/svg_to_pptx.py <project_path>
uv run python scripts/svg_to_pptx.py <project_path> --only native
uv run python scripts/svg_to_pptx.py <project_path> --only legacy
uv run python scripts/svg_to_pptx.py <template_import_output> --only native -s svg-flat
uv run python scripts/svg_to_pptx.py <project_path> --no-notes
uv run python scripts/svg_to_pptx.py <project_path> -t none
uv run python scripts/svg_to_pptx.py <project_path> --auto-advance 3
uv run python scripts/svg_to_pptx.py <project_path> --animation mixed --animation-duration 0.8
uv run python scripts/svg_to_pptx.py <project_path> --no-merge   # strict line-fidelity mode (see below)
...
- Recorded narration is opt-in:
  ...
  - Voice choices can be listed with `uv run python scripts/notes_to_audio.py --list-common-voices`, `uv run python scripts/notes_to_audio.py --list-voices --locale zh-CN`, or provider-specific `--provider <name> --list-voices`
...
```bash
uv run python scripts/total_md_split.py <project_path>
uv run python scripts/total_md_split.py <project_path> -o <output_directory>
uv run python scripts/total_md_split.py <project_path> -q
```
...
```bash
uv run python scripts/svg_quality_checker.py examples/project/svg_output/01_cover.svg
uv run python scripts/svg_quality_checker.py examples/project/svg_output
uv run python scripts/svg_quality_checker.py examples/project
uv run python scripts/svg_quality_checker.py examples/project --format ppt169
uv run python scripts/svg_quality_checker.py --all examples
uv run python scripts/svg_quality_checker.py examples/project --export
```
...
```bash
uv run python scripts/svg_position_calculator.py calc bar --data "A:185,B:142" --area "130,155,1200,480" --bar-width 120
uv run python scripts/svg_position_calculator.py calc line --data "0:50,10:80,20:120" --area "120,120,1200,600" --y-range "0,150"
uv run python scripts/svg_position_calculator.py calc pie --data "A:35,B:25,C:20" --center "420,400" --radius 200
uv run python scripts/svg_position_calculator.py calc grid --rows 2 --cols 3 --area "50,150,1230,670"
```
...
```bash
uv run python scripts/svg_position_calculator.py analyze <svg_file>
```
...
```bash
uv run python scripts/svg_finalize/flatten_tspan.py examples/<project>/svg_output
uv run python scripts/svg_finalize/flatten_tspan.py path/to/input.svg path/to/output.svg
```

### `svg_rect_to_path.py`

```bash
uv run python scripts/svg_finalize/svg_rect_to_path.py <project_path>
uv run python scripts/svg_finalize/svg_rect_to_path.py <project_path> -s final
uv run python scripts/svg_finalize/svg_rect_to_path.py path/to/file.svg
```
...
```bash
uv run python scripts/svg_finalize/align_embed_images.py path/to/slide.svg
uv run python scripts/svg_finalize/align_embed_images.py --dry-run path/to/slide.svg
```
...
runs, use `uv run python scripts/finalize_svg.py <project_path>`; the old
...
```bash
uv run python scripts/svg_finalize/embed_icons.py output.svg
uv run python scripts/svg_finalize/embed_icons.py svg_output/*.svg
uv run python scripts/svg_finalize/embed_icons.py --dry-run svg_output/*.svg
```
