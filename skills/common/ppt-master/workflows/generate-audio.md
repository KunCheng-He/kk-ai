...
- Default mode: `edge-tts` is installed (`uv run python -m # Install: cd scripts && uv sync
# Original: pip install edge-tts`).
...
```bash
uv run python skills/ppt-master/scripts/notes_to_audio.py --list-voices --locale <locale>
```

**ElevenLabs backend**:

```bash
uv run python skills/ppt-master/scripts/notes_to_audio.py --provider elevenlabs --list-voices
```

**Cloud providers using explicit voice IDs/names**:

```bash
uv run python skills/ppt-master/scripts/notes_to_audio.py --provider minimax --list-voices
uv run python skills/ppt-master/scripts/notes_to_audio.py --provider qwen --list-voices
uv run python skills/ppt-master/scripts/notes_to_audio.py --provider cosyvoice --list-voices
```
...
uv run python skills/ppt-master/scripts/notes_to_audio.py <project_path> \
...
uv run python skills/ppt-master/scripts/svg_to_pptx.py <project_path> \
...
- (If skipped embedding) one-line hint on how to embed later: `uv run python skills/ppt-master/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.
