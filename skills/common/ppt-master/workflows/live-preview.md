...
```bash
uv run python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --daemon
```
...
1. Discover annotations:
   ```bash
   uv run python ${SKILL_DIR}/scripts/check_annotations.py <project_path>
   ```
...
```bash
uv run python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --daemon --no-browser
# or for Step 6's auto-startup on a remote host:
uv run python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --live --daemon --no-browser
```
