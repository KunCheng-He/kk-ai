...
# Original: pip install playwright
uv run python -m playwright install chromium

# 2. live-preview server running for this project (provides inlined SVG fetch)
uv run python skills/ppt-master/scripts/svg_editor/server.py <project_path> --no-browser
...
```bash
uv run python skills/ppt-master/scripts/visual_review.py <project_path>
```
