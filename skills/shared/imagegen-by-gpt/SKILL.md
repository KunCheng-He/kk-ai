...
- If the user explicitly asks for a transparent image/background, stay on built-in `image_gen` first: prompt for a flat removable chroma-key background, then remove it locally with the installed helper at `./scripts/remove_chroma_key.py` from the skill root, or `"$SKILL_DIR/scripts/remove_chroma_key.py"` if the caller exposes `SKILL_DIR`.
...
- `./scripts/remove_chroma_key.py` from the skill root, or `"$SKILL_DIR/scripts/remove_chroma_key.py"`: removes a flat chroma-key background from a generated image and writes a PNG/WebP with alpha. Prefer auto-key sampling, soft matte, and despill for antialiased edges.
...
3. After generation, move or copy the selected source image from `$CODEX_HOME/generated_images/...` into the workspace or `tmp/imagegen-by-gpt/`.
4. Run the helper from the skill root, or via `SKILL_DIR` if the caller exposes it:
   ```bash
...
- Use `tmp/imagegen-by-gpt/` for intermediate files (for example JSONL batches); delete them when done.
- Write final artifacts under `output/imagegen-by-gpt/`.
...
- `./scripts/remove_chroma_key.py` from the skill root, or `"$SKILL_DIR/scripts/remove_chroma_key.py"`: local post-processing helper for built-in transparent-image requests.
