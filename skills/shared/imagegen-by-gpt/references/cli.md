...
Either run from the skill root with `python ./scripts/image_gen.py ...`, or set `SKILL_DIR` to the root of this skill and invoke the script through that variable:

```bash
export SKILL_DIR="/absolute/path/to/this/skill"
export IMAGE_GEN="$SKILL_DIR/scripts/image_gen.py"
```
...
python "$IMAGE_GEN" generate \
  --prompt "Test" \
  --out output/imagegen-by-gpt/test.png \
  --dry-run
```

Notes:
- One-off dry-runs print the API payload and the computed output path(s).
- Repo-local finals should live under `output/imagegen-by-gpt/`.
...
python "$IMAGE_GEN" generate \
  ...
  --size 1024x1024 \
  --out output/imagegen-by-gpt/alpine-cabin.png
```
...
python "$IMAGE_GEN" edit \
  ...
  --out output/imagegen-by-gpt/sunset-edit.png
```
...
- Quality: `medium`
- Output format: `png`
- Default one-off output path: `output/imagegen-by-gpt/output.png`
...
python "$IMAGE_GEN" generate \
  ...
  --quality low \
  --size 1024x1024 \
  --out output/imagegen-by-gpt/mug-draft.png
```
...
python "$IMAGE_GEN" generate \
  ...
  --quality high \
  --size 2048x1152 \
  --out output/imagegen-by-gpt/mug-hero.png
```
...
python "$IMAGE_GEN" generate \
  ...
  --size 3840x2160 \
  --quality high \
  --out output/imagegen-by-gpt/architecture-4k.png
```
...
python "$IMAGE_GEN" generate \
  ...
  --output-format png \
  --out output/imagegen-by-gpt/product-cutout.png
```
...
python "$IMAGE_GEN" edit \
  ...
  --quality high \
  --input-fidelity high \
  --out output/imagegen-by-gpt/background-edit.png
```
...
## Output handling
- Use `tmp/imagegen-by-gpt/` for temporary JSONL inputs or scratch files.
- Use `output/imagegen-by-gpt/` for final outputs.
...
```bash
mkdir -p tmp/imagegen-by-gpt output/imagegen-by-gpt/batch
cat > tmp/imagegen-by-gpt/prompts.jsonl << 'EOF'
...
python "$IMAGE_GEN" generate-batch \
  --input tmp/imagegen-by-gpt/prompts.jsonl \
  --out-dir output/imagegen-by-gpt/batch \
  --concurrency 5

rm -f tmp/imagegen-by-gpt/prompts.jsonl
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `./scripts/remove_chroma_key.py` from the skill root, or `"$SKILL_DIR/scripts/remove_chroma_key.py"`
