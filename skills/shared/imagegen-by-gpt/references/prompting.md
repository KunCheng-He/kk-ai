...
- After generation, remove the background locally with `python ./scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` from the skill root, or `python "$SKILL_DIR/scripts/remove_chroma_key.py" ...` when the caller exposes `SKILL_DIR`, and validate the alpha result before shipping it.
